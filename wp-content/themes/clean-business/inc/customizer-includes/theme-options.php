...
$layouts = clean_business_layouts();
$choices = array();
...
$layouts = clean_business_get_archive_content_layout();
$choices = array();
foreach ( $layouts as $layout ) {
	$choices[ $layout['value'] ] = $layout['label'];
}
...
$navigation_description = sprintf(
	wp_kses(
		__( '<a target="_blank" href="%s">WP-PageNavi Plugin</a> is recommended for Numeric Option(But will work without it).<br/>Infinite Scroll Options requires <a target="_blank" href="%s">JetPack Plugin</a> with Infinite Scroll module Enabled.', 'clean-business' ),
		array(
			'a' => array(
				'href' => array(),
				'target' => array(),
...
$pagination_types = clean_business_get_pagination_types();
$choices = array();
