 ...
 * Custom Tag Cloud Widget
 */
class clean_business_tag_cloud_widget extends WP_Widget {
	...
	function __construct() {
		...
		$widget_ops = array(
			...
			'description' => esc_html__( 'Displays Custom Tag Cloud with selected layout', 'clean-business' ),
		);
		...
		parent::__construct(
			...
			esc_html__( 'CT: Tag Cloud', 'clean-business' ), // Name
			$widget_ops,
			$control_ops
		);
	}
	...
	function form($instance) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'no_of_tags' ); ?>"><?php esc_html_e( 'No. of Items', 'clean-business' ); ?>:</label>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'skins' ); ?>"><?php esc_html_e( 'Skins(Theme)', 'clean-business' ); ?>:</label>
			<select class="ct_feat_post_post_type widefat" id="<?php echo $this->get_field_id( 'skins' ); ?>" name="<?php echo $this->get_field_name( 'skins' ); ?>">
				<?php
					$post_type_choices = array(
						'default'         => esc_html__( 'Default', 'clean-business' ),
						'box'             => esc_html__( 'Box', 'clean-business' ),
						'rounded-corners' => esc_html__( 'Rounded Corners', 'clean-business' ),
					);
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'sequence' ); ?>"><?php esc_html_e( 'Sequence', 'clean-business' ); ?>:</label>
			<select class="ct_feat_post_post_type widefat" id="<?php echo $this->get_field_id( 'sequence' ); ?>" name="<?php echo $this->get_field_name( 'sequence' ); ?>">
				<?php
					$post_type_choices = array(
						...
						'ASC'  => esc_html__( 'Ascending', 'clean-business' ),
						'DESC' => esc_html__( 'Descending', 'clean-business' ),
					);
		...
		<h4><?php esc_html_e( 'Taxonomies to be used', 'clean-business' ); ?></h4>
		<p>
        	...
        	<label for="<?php echo $this->get_field_id('post_tag'); ?>"><?php esc_html_e( 'Post Tags', 'clean-business' ); ?></label><br />
        </p>
        <p>
        	...
        	<label for="<?php echo $this->get_field_id('category'); ?>"><?php esc_html_e( 'Category', 'clean-business' ); ?></label><br />
        </p>
        <p>
        	...
        	<label for="<?php echo $this->get_field_id('link_category'); ?>"><?php esc_html_e( 'Link Category', 'clean-business' ); ?></label><br />
        </p>
        <p>
        	<label for="<?php echo $this->get_field_id( 'custom_taxonomy' ); ?>"><?php esc_html_e( 'Custom Taxnomy', 'clean-business' ); ?>:</label>
			...
			<span class="description"><?php esc_html_e( 'Use Custom Taxonomy Slug separated by comma(,)', 'clean-business' ); ?></span>
        </p>

		<h2><?php esc_html_e( 'Additional Options', 'clean-business' ); ?></h2>
		<hr>

		<p>
			<label for="<?php echo $this->get_field_id( 'smallest' ); ?>"><?php esc_html_e( 'Smallest', 'clean-business' ); ?>:</label>
			...
			<br/>
			<span class="description"><?php esc_html_e( 'The text size of the tag with the smallest count value (units given by unit parameter). ', 'clean-business' ); ?></span>
		</p>

		<p>
			<label for="<?php echo $this->get_field_id( 'largest' ); ?>"><?php esc_html_e( 'Largest', 'clean-business' ); ?>:</label>
			...
			<br/>
			<span class="description"><?php esc_html_e( 'The text size of the tag with the largest count value (units given by unit parameter). ', 'clean-business' ); ?></span>
		</p>

		<p>
			<label for="<?php echo $this->get_field_id( 'unit' ); ?>"><?php esc_html_e( 'Unit', 'clean-business' ); ?>:</label>
			<select class="small-text" id="<?php echo $this->get_field_id( 'unit' ); ?>" name="<?php echo $this->get_field_name( 'unit' ); ?>">
				<?php
					$post_type_choices = array(
						'pt' => esc_html__( 'pt', 'clean-business' ),
						'px' => esc_html__( 'px', 'clean-business' ),
						'em' => esc_html__( 'em', 'clean-business' ),
						'%'  => esc_html__( '%', 'clean-business' ),
					);
			...
			<br/>
			<span class="description"><?php esc_html_e( 'Unit of measure as pertains to the smallest and largest values. This can be any CSS length value.', 'clean-business' ); ?></span>
		</p>
	...
	function update( $new_instance, $old_instance ) {
		...
		$instance['post_tag']        = clean_business_sanitize_checkbox( $new_instance['post_tag'] );
		$instance['category']        = clean_business_sanitize_checkbox( $new_instance['category'] );
		$instance['link_category']   = clean_business_sanitize_checkbox( $new_instance['link_category'] );
	...
	function widget( $args, $instance ) {
		...
		if( '' != $instance['custom_taxonomy'] ) {
			$custom_taxonomy = array_map( 'trim', explode( ',', (array) $instance['custom_taxomony'] ) );

			$taxonomy = array_push( $taxonomy, $custom_taxonomy );
		}
...
}// end clean_business_tagcloud_widget class

//From Codex: https://codex.wordpress.org/Widgets_API
add_action('widgets_init',
	create_function('', 'return register_widget("clean_business_tag_cloud_widget");')
);
