...
function clean_business_featured_content_display() {
	...
	if ( $enable_content == 'entire-site' || ( ( is_front_page() || ( is_home() && $page_for_posts != $page_id ) ) && $enable_content == 'homepage' ) ) {
		...
		$output .= '
			...
			if ( has_post_thumbnail() ) {
				...
				<figure class="featured-image">
					...
					'. get_the_post_thumbnail( $post->ID, 'clean-business-featured-content', array( 'title' => esc_attr( $title_attribute ), 'alt' => esc_attr( $title_attribute ) ) ) .'
					</a>
				</figure>';
			}
			else {
				$image = '<img class="no-featured-image wp-post-image" src="'.get_template_directory_uri().'/images/gallery/no-featured-image-1920x800.jpg" >';
