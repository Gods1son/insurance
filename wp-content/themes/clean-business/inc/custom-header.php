...
if ( ! function_exists( 'clean_business_featured_overall_image' ) ) :
	...
	function clean_business_featured_overall_image() {
		...
		if ( is_page() || is_single() ) {
			...
			if ( $individual_featured_image == 'disabled' || ( $individual_featured_image == 'default' && $enable_header_image == 'disabled' ) ) {
				echo '<!-- Page/Post Disable Header Image -->';
				return;
			}
			elseif ( $individual_featured_image == 'enable' && $enable_header_image == 'disabled' ) {
				clean_business_featured_page_post_image();
			}
		}

		// Check Homepage
		if ( $enable_header_image == 'homepage' ) {
			if ( is_front_page() || ( is_home() && $page_for_posts != $page_id ) ) {
				clean_business_featured_image();
			}
		}
		// Check Excluding Homepage
		if ( $enable_header_image == 'exclude-home' ) {
			if ( is_front_page() || ( is_home() && $page_for_posts != $page_id ) ) {
				return false;
			}
			else {
		...
		elseif ( $enable_header_image == 'exclude-home-page-post' ) {
			if ( is_front_page() || ( is_home() && $page_for_posts != $page_id ) ) {
				return false;
			}
		...
		// Check Entire Site
		elseif ( $enable_header_image == 'entire-site' ) {
			clean_business_featured_image();
		}
		// Check Entire Site (Post/Page)
		elseif ( $enable_header_image == 'entire-site-page-post' ) {
			if ( is_page() || is_single() ) {
				clean_business_featured_page_post_image();
			}
			else {
		...
		// Check Page/Post
		elseif ( $enable_header_image == 'pages-posts' ) {
			if ( is_page() || is_single() ) {
				clean_business_featured_page_post_image();
			}
		}
